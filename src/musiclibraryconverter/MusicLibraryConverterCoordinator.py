...
def createWorker (verbose, metadataonly, converter, event, srcFile, dstFile):
    worker = MusicLibraryConverterWorker(converter, event, verbose, metadataonly, srcFile, dstFile)
    worker.run()
...
class MusicLibraryConverterCoordinator(object):
    '''
    classdocs
    '''
    __instance = None

    def __init__(self, verbose: int, recursive: bool, threads: int, includePattern: str, excludePattern: str, overwrite: bool, overwriteIfSrcNewer: bool, metadataonly: bool, src: str, dst: str, converterType: str='FFMpeg', srcParams: str='', dstParams: str='', srcExt: str='flac', dstExt='', dstCodec: str=''):
        '''
        Constructor
        '''
...
#         print ("dst="+dst)
        ...
        if dstCodec=='':
            if self.__dstExt.endswith('mp3'):
                self.__dstCodec='mp3'
            elif self.__dstExt.endswith('ogg'):
                self.__dstCodec='vorbis'
        else:
            self.__dstCodec = dstCodec
        self.__converter = MusicLibraryConverterBackendFactory(converterType, srcParams, dstParams, dstCodec=self.__dstCodec)
        if self.__converter == None or not self.__converter.backendFound():
            raise ConverterBackendNotFoundException(converterType)
        
    def handleFiles(self, srcFile: Path, dst: Path):
        ':type src: Path'
        ':type dst: Path'
    ...
    def copyFile(self, srcFile: Path, dstDir: Path):
        ':type file: Path'
        ':type dst: Path'
    ...
    def handlePathRecursively(self, src: Path, dst: Path):
        ':type src: Path'
        ':type dst: Path'
        if not src.exists():
    ...
    def deriveDstFile(self, src: Path, dst: Path):
        ':type src: Path'
        ':type dst: Path'
        if dst.is_file():
            return dst
